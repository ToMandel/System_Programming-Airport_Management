...
int	saveFlightToBinaryFileCompressed(const Flight* pF, FILE* fp)
{
	if (!writeCharsToFile(pF->originCode, 3, fp, "error loading origin code"))
		return 0;
	if (!writeCharsToFile(pF->destCode, 3, fp, "error loading destination code"))
		return 0;
	int data = 0;
	data = (pF->date.year & 0x3FFFF) << 14 | (pF->date.month & 0xF) << 10 | (pF->date.day & 0x1F) << 5 | (pF->hour & 0x1F);
	if (fwrite(&data, sizeof(int), 1, fp) != 1)
		return 0;
	return 1;
}

int unCompressedFlight(FILE* fp, Flight* pFlight)
{
	if (!readCharsFromFile(pFlight->originCode, 3, fp, "error in reading origin code"))
		return 0;
	pFlight->originCode[CODE_LENGTH + 1] = '\0';
	if (!readCharsFromFile(pFlight->destCode, 3, fp, "error in reading destination code"))
		return 0;
	pFlight->destCode[CODE_LENGTH + 1] = '\0';
	int data;
	if (fread(&data, sizeof(int), 1, fp) != 1)
		return 0;
	pFlight->date.year = (data >> 14) & 0x3FFFF;
	pFlight->date.month = (data >> 10) & 0xF;
	pFlight->date.day = (data >> 5) & 0x1F;
	pFlight->hour = data & 0X1F;
	return 1;
}
